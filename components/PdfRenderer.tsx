...
type PdfRenderedProps = {
  url: string;
};

pdfjs.GlobalWorkerOptions.workerSrc = `//unpkg.com/pdfjs-dist@${pdfjs.version}/build/pdf.worker.min.mjs`;

function PdfRenderer({ url }: PdfRenderedProps) {
      ...
      <div className="h-14 w-full border-b border-zinc-200 flex items-center justify-between px-2">
        <div className="flex items-center gap-1.5">
          <Button
            disabled={curPage <= 1}
            onClick={() => {
              const newPage = curPage - 1 > 1 ? curPage - 1 : 1;
              setCurPage(newPage);
          ...
          <Button
            ...
            onClick={() => {
              const newPage = curPage + 1 > numPages! ? numPages! : curPage + 1;
              setCurPage(newPage);
              setValue("page", String(newPage));
            }}
            variant="ghost"
        ...
        <div className="space-x-2">
          ...
          <Button
            ...
            variant="ghost"
            area-label="rotate 90deg"
          >
            <RotateCw className="h-4 w-4" />
          </Button>

          <PdfFullscreen curPage={curPage} url={url} />
        </div>
      </div>
